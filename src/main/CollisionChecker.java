	...
	public void checkTile(Entity entity, boolean checkForFall) {
		...
		if(checkForFall) {			
			if(entity.isFalling)	{
				...
				tileNum1 = gp.tileH[gp.mapId].mapTileNum[entityLeftCol][entityBottomRow];
...
//					entity.isLeapingRight = false;
		...
		case "up":
			if(entityTopY - entity.speed < 0)
				...
				tileNum1 = gp.tileH[gp.mapId].mapTileNum[entityLeftCol][entityTopRow];
				tileNum2 = gp.tileH[gp.mapId].mapTileNum[entityRightCol][entityTopRow];
...
//					entity.collisionOn = true;	
		...
		case "down":
			if(entityBottomY + entity.speed >= gp.screenHeight)
				...
				tileNum1 = gp.tileH[gp.mapId].mapTileNum[entityLeftCol][entityBottomRow];
				tileNum2 = gp.tileH[gp.mapId].mapTileNum[entityRightCol][entityBottomRow];
				entity.collisionOn = true;
				if( (tileNum1 == 3 && entityLeftX - entityLeftCol*gp.tileSize <= 36) || (tileNum2 == 3 && (entityRightCol+1)*gp.tileSize - entityRightX <= 36) )
				{
						entity.collisionOn = false;
...
//					entity.collisionOn = true;
		...
		case "left":
			if(entityLeftX - entity.speed < 0)
				...
				tileNum1 = gp.tileH[gp.mapId].mapTileNum[entityLeftCol][entityTopRow];
				tileNum2 = gp.tileH[gp.mapId].mapTileNum[entityLeftCol][entityBottomRow];
//				System.out.println(tileNum1+" "+tileNum2);
...
//				}
				...
				if( tileNum1 == 1 || tileNum1 == 3 )	{
					...
					if(entity.up1 == null) { // monster
						...
						if(gp.tileH[gp.mapId].mapTileNum[entityRightCol][entityTopRow] == 3) {
						...
						if(gp.tileH[gp.mapId].mapTileNum[entityRightCol][entityTopRow] == 1) {
							entity.x -= entity.speed;
							entity.isLeapingLeft = true;
						}
						entity.collisionOn = false;
//						entity.isFalling = true;
		...
		case "right":
			...
			if(entity.x + gp.tileSize + entity.speed >= 1366) // my screen width(1366)
				...
				tileNum1 = gp.tileH[gp.mapId].mapTileNum[entityRightCol][entityTopRow];
				tileNum2 = gp.tileH[gp.mapId].mapTileNum[entityRightCol][entityBottomRow];
//				if(entity.up1 != null) { // player
...
//				}
				...
				if( tileNum1 == 1 || tileNum1 == 3 )	{
					...
					if(entity.up1 == null) { // monster
						...
						if(gp.tileH[gp.mapId].mapTileNum[entityLeftCol][entityTopRow] == 3) {
							entity.x += entity.speed;
						...
						if(gp.tileH[gp.mapId].mapTileNum[entityLeftCol][entityTopRow] == 1) {
							entity.x += entity.speed;
							entity.isLeapingRight = true;
						}
						entity.collisionOn = false;
	...
	public int checkCoin(Entity entity, int mapId, SuperObject[][] obj)	{
		int index = -1;
	    for (int i = 0; i < obj[mapId].length && index == -1; i++)
	    {
	        if (obj[mapId][i] != null)
	        {
	            
	            if ( ((entity.x >= obj[mapId][i].x && entity.x <= obj[mapId][i].x + gp.tileSize) && (entity.y >= obj[mapId][i].y && entity.y <= obj[mapId][i].y + gp.tileSize)) || 
	            		((entity.x + gp.tileSize >= obj[mapId][i].x && entity.x + gp.tileSize <= obj[mapId][i].x + gp.tileSize) && 
	            		(entity.y + gp.tileSize >= obj[mapId][i].y && entity.y + gp.tileSize <= obj[mapId][i].y + gp.tileSize)) )
	            {
	                    index = i;
	            }
	        }
	...
	public int checkEntity(Entity entity, int mapId, Entity[][] monster) {
		int index = -1;
	    for (int i = 0; i < monster[mapId].length && index == -1; i++)
	    {
	        if (monster[mapId][i] != null)
	        {
	            
	            if ( ((entity.x >= monster[mapId][i].x && entity.x <= monster[mapId][i].x + gp.tileSize) && (entity.y >= monster[mapId][i].y && entity.y <= monster[mapId][i].y + gp.tileSize))
	            		|| ((entity.x + gp.tileSize >= monster[mapId][i].x && entity.x + gp.tileSize <= monster[mapId][i].x + gp.tileSize) && 
	            		(entity.y + gp.tileSize >= monster[mapId][i].y && entity.y + gp.tileSize <= monster[mapId][i].y + gp.tileSize)) )
	            {
	                    index = i;
	            }
	        }
